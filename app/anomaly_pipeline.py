# app/anomaly_detection_pipeline.py
"""
Complete Time-Series Anomaly Detection Pipeline
Trains on good.csv (normal data) and detects anomalies in bad.csv
"""
...
class AnomalyDetectionPipeline:
    ...
    def preprocess_data(self):
        ...
        # Extract values (train on good, test on bad)
        ...
        # For simple methods: use raw values
    ...
    def create_sequences(self, data, seq_len, stride=1):
        """Create sliding window sequences"""
        n_sequences = (len(data) - seq_len) // stride + 1
        sequences = np.zeros((n_sequences, seq_len, 1), dtype='float32')
        
        for i in range(n_sequences):
            start = i * stride
            sequences[i, :, 0] = data[start:start+seq_len, 0]
    ...
    def detect_moving_average(self, window=20, threshold=2.5):
        """Moving average based detection"""
        train_series = pd.Series(self.X_train_scaled.flatten())
        train_ma = train_series.rolling(window=window, center=True).mean()
        train_std = train_series.rolling(window=window, center=True).std()
        
        test_series = pd.Series(self.X_test_scaled.flatten())
        test_ma = test_series.rolling(window=window, center=True).mean()
        
        deviations = np.abs(test_series - test_ma)
        threshold_value = train_std.mean() * threshold
        predictions = (deviations > threshold_value).astype(int).values
        
        self.results['Moving Average'] = {
            'predictions': predictions,
            'scores': deviations.values,
    ...
    def plot_results(self):
        ...
        for method_name, result in self.results.items():
            ...
            # Left: Time series with anomalies
            normal_idx = np.where(predictions == 0)[0]
            anomaly_idx = np.where(predictions == 1)[0]
            
            # Sample for better visualization (max 2000 points)
        ...
        fig.update_layout(
            ...
            title_text="Anomaly Detection Results - All Methods",
            showlegend=False
        )
    ...
    def run_all(self):
        ...
        print("="*80)
        self.detect_zscore(threshold=3)
        self.detect_iqr(multiplier=1.5)
        self.detect_moving_average(window=20, threshold=2.5)
        
        # Machine Learning methods
        print("\n" + "="*80)
        print("MACHINE LEARNING METHODS")
        print("="*80)
        # Using contamination=0.01 (expect 1% anomalies)
...
if __name__ == "__main__":
    ...
    print("NEXT STEPS:")
    print("="*80)
    print("1. For deep learning methods (LSTM Autoencoder), use model.py")
    print("2. Adjust contamination parameter (0.01 = 1% expected anomalies)")
    print("3. Check anomaly_results.html for interactive visualization")
    print("4. Methods with ~1% anomaly rate are well-calibrated")